...
def apply_safe_actions(job_id: str, eda: Dict[str, Any], modeling: Dict[str, Any], framing: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not SAFE_AUTO_ACTIONS:
        return framing
    try:
        task = modeling.get("task") or ("classification" if (eda.get("target_type") == "categorical") else None)
        if task != "classification":
            return framing
        target = framing.get("target") or eda.get("target")
        if not target:
            return framing
        class_counts = (eda.get("target_counts") or {}).get(str(target)) or eda.get("class_counts")
        if isinstance(class_counts, dict) and sum(class_counts.values()) > 0:
            ...
            if minority < 0.2:
                ...
                model_decision(job_id, "Auto-action: set class_weight=balanced due to imbalance")
