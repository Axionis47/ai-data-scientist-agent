...
def load_sampled_chunked_csv(
    path: Path, delimiter: str, sample_target: int
) -> Dict[str, Any]:
    ...
    for chunk in pd.read_csv(path, sep=delimiter, chunksize=20_000):
        ...
        if need > 0:
            chunks.append(
                chunk.sample(n=need, random_state=42) if need < len(chunk) else chunk
            )
...
def compute_eda(df: pd.DataFrame, max_topk: int = 5) -> Dict[str, Any]:
    ...
    eda["missing"] = {
        str(c): {
            "count": int(miss[c]),
            "pct": float((miss[c] / rows * 100) if rows else 0.0),
        }
        for c in df.columns
    }
    ...
    if num_cols:
        ...
        eda["numeric_stats"] = {
            c: {k: _to_py(v.get(c)) for k, v in desc.items()} for c in num_cols
        }
        try:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("EDA: top_correlations failed: %s", e)
    ...
    try:
        ...
        logging.getLogger(__name__).warning("EDA: categorical_topk failed: %s", e)
    # Rare levels
    ...
    try:
        ...
        for c in cat_cols:
            ...
            rare_levels[str(c)] = [str(k) for k, v in vc.items() if v <= threshold][
                :max_topk
            ]
    ...
    if any(m["pct"] > 30 for m in eda["missing"].values()):
        recs.append(
            "High missingness detected; consider imputation or dropping columns >30% missing."
        )
    if any(len(v) > 0 for v in rare_levels.values()):
        recs.append(
            "High-cardinality categories/rare levels present; consider top-k + 'Other' / hashing."
        )
    if eda.get("top_correlations"):
        recs.append(
            "Strongly correlated numeric pairs; consider collinearity handling."
        )
    if id_candidates:
        recs.append("ID-like columns found; exclude from modeling to avoid leakage.")
    if time_like or eda.get("time_columns"):
        recs.append(
            "Time data detected; prefer time-based split and check trend/seasonality."
        )
...
def compute_target_relations(
    ...
    cat_cols = [
        ...
        for c in df.columns
        if c != target
        and (ptypes.is_object_dtype(df[c]) or isinstance(df[c].dtype, CategoricalDtype))
    ]
    for c in cat_cols:
        try:
...
def compute_timeseries_hints(
    df: pd.DataFrame, time_col: str, metric_col: Optional[str]
) -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            metric_col
            and metric_col in df.columns
            and ptypes.is_numeric_dtype(df[metric_col])
        ):
...
def generate_basic_plots(
    job_id: str, df: pd.DataFrame, plots_dir: Path
) -> Dict[str, Any]:
    ...
    for c in num_cols:
        ...
        plt.title(f"Hist: {c}")
        plt.tight_layout()
        p = plots_dir / f"hist_{_slug(c)}.png"
        plt.savefig(p)
        plt.close()
        hist_urls.append(f"{base_url}/hist_{_slug(c)}.png")
    # Categorical bars
    from pandas.api.types import CategoricalDtype
    ...
    cat_cols = [
        c
        for c in df.columns
        if (ptypes.is_object_dtype(df[c]) or isinstance(df[c].dtype, CategoricalDtype))
    ][:PLOTS_PER_CATEG]
    bar_urls: List[str] = []
    for c in cat_cols:
        plt.figure(figsize=(4, 3))
        ...
        plt.title(f"Top: {c}")
        plt.tight_layout()
        p = plots_dir / f"bar_{_slug(c)}.png"
        plt.savefig(p)
        plt.close()
        bar_urls.append(f"{base_url}/bar_{_slug(c)}.png")
    # Missingness bar
    plt.figure(figsize=(5, 3))
    miss_pct = (df.isna().mean() * 100).sort_values(ascending=False).head(20)
    miss_pct.plot(kind="bar")
    plt.title("Missingness % (top 20)")
    plt.tight_layout()
    miss_path = plots_dir / "missingness.png"
    plt.savefig(miss_path)
    plt.close()
    return {
        "histograms": hist_urls,
        "categoricals": bar_urls,
        "missingness": f"{base_url}/missingness.png",
    }


def generate_basic_plots_storage(
    storage, job_id: str, df: pd.DataFrame
) -> Dict[str, Any]:
    ...
    plt.close()
    storage.put_file(
        job_id, "plots/missingness.png", buf.getvalue(), content_type="image/png"
    )

    return {
        "histograms": hist_urls,
        "categoricals": bar_urls,
        "missingness": storage.url_for(job_id, "plots/missingness.png"),
    }
